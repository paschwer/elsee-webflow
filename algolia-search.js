window.addEventListener("DOMContentLoaded", () => {
  ...
  // état global front
  const selectedFacetTags = new Set(); // toutes les facettes "normales" qu’on clique
  const selectedJobTags = []; // liste des métiers choisis
  ...
  function initAlgolia() {
    ...
    // -----------------------------------------------------------
    // widget custom pour tout dessiner (tags etc.)
    // -----------------------------------------------------------
    const dynamicSuggestionsWidget = {
      render({ results }) {
        ...
        // on met d'abord celles déjà sélectionnées
        selectedSpe.forEach((value) => {
          seen.add(value);
          speBlocks.push({ name: value, selected: true });
        });
        ...
        for (const fv of speFacetValues) {
          ...
          speBlocks.push({ name: fv.name, selected: false });
          seen.add(fv.name);
        }
        ...
        if (jobFilterWrapper) {
          ...
          const jobListHtml = mergedArr
            ...
            .map((item) => {
              ...
              const key = `jobs:::${cleanName}`;
              const isSelected =
                selectedFacetTags.has(key) ||
                selectedJobTags.includes(cleanName);
              return (
                '<div class="directory_category_tag_wrapper ' +
                (isSelected ? "is-selected" : "") +
        ...
        if (discountFilterWrapper) {
          ...
          console.log("[ALGOLIA] facets bruts remboursement :", reimburseFacetValues);
          ...
          console.log("[ALGOLIA] facets remboursement après tri :", filtered);
  ...
  // -----------------------------------------------------------
  // clics sur suggestions dropdown
  // -----------------------------------------------------------
  ...
  // -----------------------------------------------------------
  // clics sur jobs (fusion mainjob + jobs)
  // -----------------------------------------------------------
  ...
  // -----------------------------------------------------------
  // fonction appelée depuis ton script maps
  // -----------------------------------------------------------
  ...
  // -----------------------------------------------------------
  // lecture des paramètres d’URL au chargement
  // -----------------------------------------------------------
